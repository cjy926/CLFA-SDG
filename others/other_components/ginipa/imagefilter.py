...
class GradlessGCReplayNonlinBlock(nn.Module):
    ...
    def forward(self, x_in, requires_grad=False):
        ...
        ker = torch.randn([self.out_channel * nb, self.in_channel, k, k], requires_grad=self.requires_grad).to(device=self.device)
        shift = torch.randn([self.out_channel * nb, 1, 1], requires_grad=self.requires_grad).to(device=self.device) * 1.0
...
class GINGroupConv(nn.Module):
    ...
    def forward(self, x_in):
        ...
        alphas = alphas.repeat(1, nc, 1, 1).to(device=self.device)  # nb, nc, 1, 1
